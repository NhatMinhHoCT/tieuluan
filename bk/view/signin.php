...
if (isset($_SESSION['notification']) && ($_SESSION['notification'] == 1)) {
  echo "<script>
    setTimeout(function() {
      showToast('Thông báo', 'Đăng ký tài khoản thành công', 'info');
    }, 200);
  </script>";
...
if (isset($_POST['dangnhap'])) {
  ...
  if ($list->checklogin($username, $password)) {
    ...
    echo "<script>
    setTimeout(function() {
      showToast('Thông báo', 'Đăng nhập không thành công', 'error');
    }, 200);
  </script>";
  }
}
    ...
    <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-4">
      <div class="bg-light rounded p-4 p-sm-5 my-4 mx-3">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>">
          ...
          <p class="text-center  mb-0">Chưa có tài khoản? <a href="signup.php" class=" fw-bold fs-5 ">Đăng ký</a></p>
        </form>
      </div>
    </div>
  </div>
